...
namespace TUIO
{
     ...
     * @version 1.4
     * 
     * Hand extensions added by Isaac Liao 11/2012.
     */
    public class TuioClient
    {
        ...
        private object handSync = new object();
        ...
        private Dictionary<long, TuioHand> handList = new Dictionary<long, TuioHand>(32);
        private List<long> aliveHandList = new List<long>(32);
        private List<long> newHandList = new List<long>(32);

        private List<TuioObject> frameObjects = new List<TuioObject>(32);
        private List<TuioCursor> frameCursors = new List<TuioCursor>(32);
        private List<TuioHand> frameHands = new List<TuioHand>(32);
        ...
        private void processMessage(OSCMessage message)
        {
            ...
            //if (command == "set" & address == "/tuio/2Dobj")
            //{
            ...
            if (address == "/tuio/_hand")
            {  // Added by Isaac to interpret hand extension messages
                ...
                if (command == "set")
                {
                    ...
                    float mspeed = (float)args[6]; // length of motion vector given by xspeed,yspeed (which are normalized)
                    int f_id1 = (int)args[7];      // finger id's belonging to this hand; if less than 5, inactive finger id's set to -1
                    int f_id2 = (int)args[8];
                    int f_id3 = (int)args[9];
                    int f_id4 = (int)args[10];
                    int f_id5 = (int)args[11];


                    // Denormalize the velocity vector because that's how TuioContainer stores them
                    float velocityscale = mspeed / (float)Math.Sqrt(xspeed * xspeed + yspeed * yspeed);
                    xspeed = xspeed * velocityscale;
                    yspeed = yspeed * velocityscale;

                    lock (handSync)
                    {
                        if (!handList.ContainsKey(h_id))
                        {
                            TuioHand addHand = new TuioHand(h_id, xpos, ypos, f_id1, f_id2, f_id3, f_id4, f_id5);
                            frameHands.Add(addHand);
                        }
                        else
                        {
                            TuioHand thand = handList[h_id];
                            if (thand == null) return;
                            if ((thand.getX() != xpos) || (thand.getY() != ypos) || (thand.getXSpeed() != xspeed) || (thand.getYSpeed() != yspeed))
                            {

                                TuioHand updateHand = new TuioHand(h_id, xpos, ypos);
                                updateHand.update(xpos, ypos, xspeed, yspeed, f_id1, f_id2, f_id3, f_id4, f_id5);
                                frameHands.Add(updateHand);
                            }
                        }
                    }
                }
                else if (command == "alive")
                {
                    ...
                    for (int i = 1; i < args.Count; i++)
                    {
                        ...
                        newHandList.Add(s_id);
                        // reduce the object list to the lost objects
                        if (aliveHandList.Contains(s_id))
                            aliveHandList.Remove(s_id);
                    }

                    // remove the remaining objects
                    lock (handSync)
                    {
                        for (int i = 0; i < aliveHandList.Count; i++)
                        {
                            long s_id = aliveHandList[i];
                            if (handList.ContainsKey(s_id)) // Check if handList contains the key
                            {
                                TuioHand removeHand = handList[s_id];
                                removeHand.remove(currentTime);
                                frameHands.Add(removeHand);
                            }
                        }
                    }
                ...
                else if (command == "fseq")
                {
                    ...
                    if (!lateFrame)
                    {
                        ...
                        IEnumerator<TuioHand> frameEnum = frameHands.GetEnumerator();
                        while (frameEnum.MoveNext())
                        {
                            TuioHand tobj = frameEnum.Current;

                            switch (tobj.getTuioState())
                            {
                                case TuioHand.TUIO_REMOVED:
                                    TuioHand removeHand = tobj;
                                    removeHand.remove(currentTime);

                                    for (int i = 0; i < listenerList.Count; i++)
                                    {
                                        TuioListener listener = (TuioListener)listenerList[i];
                                        if (listener != null) listener.removeTuioHand(removeHand);
                                    }
                                    lock (handSync)
                                    {
                                        handList.Remove(removeHand.getSessionID());
                                    }
                                    break;
                                case TuioHand.TUIO_ADDED:
                                    TuioHand addHand = new TuioHand(currentTime, tobj.getHandID(), tobj.getX(), tobj.getY(), tobj.getFingerID1(), tobj.getFingerID2(), tobj.getFingerID3(), tobj.getFingerID4(), tobj.getFingerID5());
                                    lock (handSync)
                                    {
                                        if (handList.ContainsKey(addHand.getSessionID())) // Added by Isaac to resolve HandAlreadyAdded exceptions
                                            handList.Remove(addHand.getSessionID());      // 
                                        handList.Add(addHand.getSessionID(), addHand);
                                    }
                                    for (int i = 0; i < listenerList.Count; i++)
                                    {
                                        TuioListener listener = (TuioListener)listenerList[i];
                                        if (listener != null) listener.addTuioHand(addHand);
                                    }
                                    break;
                                default:
                                    TuioHand updateHand = getTuioHand(tobj.getSessionID());
                                    if ((tobj.getX() != updateHand.getX() && tobj.getXSpeed() == 0) || (tobj.getY() != updateHand.getY() && tobj.getYSpeed() == 0))
                                        updateHand.update(currentTime, tobj.getX(), tobj.getY());
                                    else
                                        updateHand.update(currentTime, tobj.getX(), tobj.getY(), tobj.getXSpeed(), tobj.getYSpeed(), tobj.getFingerID1(), tobj.getFingerID2(), tobj.getFingerID3(), tobj.getFingerID4(), tobj.getFingerID5());

                                    for (int i = 0; i < listenerList.Count; i++)
                                    {
                                        TuioListener listener = (TuioListener)listenerList[i];
                                        if (listener != null) listener.updateTuioHand(updateHand);
                                    }
                                    break;
                            }
                        }
                        ...
                        List<long> buffer = aliveHandList;
                        aliveHandList = newHandList;
                        // recycling the List
                        newHandList = buffer;
                    }
                    frameHands.Clear();
                        ...
                        while (frameEnum.MoveNext())
                        {
                            ...
                            switch (tobj.getTuioState())
                            {
                                ...
                                case TuioObject.TUIO_ADDED:
                                    ...
                                    lock (objectSync)
                                    {
                                        if (objectList.ContainsKey(addObject.getSessionID())) // Added by Isaac to resolve ObjectAlreadyAdded exceptions
                                            objectList.Remove(addObject.getSessionID());      // 
         ...
         * @return an active TuioObject corresponding to the provided Session ID or NULL
         */
        public TuioHand getTuioHand(long s_id)
        {
            TuioHand thand = null;
            lock (handSync)
            {
                handList.TryGetValue(s_id, out thand);
            }
            return thand;
        }
